...
function validate(num) {
  ...
  if (typeof num === "number" && remainder === 0 && num >= 100) {
    return true;
  } else {
    return false;
  }
...
function tidyUpStrings(arrayOfStrings) {
  let formattedStrings = arrayOfStrings;
  let tidyUp = "";
  for (let i = 0; i < formattedStrings.length; i++) {
    tidyUp = formattedStrings.shift().trim().toLowerCase().replace("/", "");
    formattedStrings.push(tidyUp);
  }
